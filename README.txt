...
3. Automatic import runs once on activation:
   ...
   - Bundled media assets located in `assets/uploads/` are copied into the site’s `wp-content/uploads/` directory when missing, so imagery and Elementor-generated CSS become available automatically.
...
-----------------------
- The importer runs only once. To trigger it again, delete the `marble_replica_import_completed` option (via WP-CLI or the Options table) and reactivate the theme. Bundled uploads copy again for any missing files.
...
- Bundled uploads are copied on activation, but media items are not programmatically registered; WordPress will serve them directly from `/wp-content/uploads/…`. If needed, use a media import plugin (e.g., “Add From Server”) to register attachments.
