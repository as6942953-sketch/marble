...
/**
 * Copy images from theme to WordPress uploads folder on activation
 * This ensures all images are accessible at the correct wp-content/uploads paths
 */
function marble_theme_copy_images_to_uploads() {
	$theme_images_dir = get_template_directory() . '/assets/uploads/';
	$wp_uploads_dir = wp_upload_dir();
	$wp_uploads_base = $wp_uploads_dir['basedir'];
	
	// Check if theme images directory exists
	if ( ! is_dir( $theme_images_dir ) ) {
		return;
	}
	
	// Check if we've already copied images (to avoid re-copying on every activation)
	$copied_marker = get_option( 'marble_theme_images_copied', false );
	
	if ( $copied_marker ) {
		// Images already copied, skip
		return;
	}
	
	// Function to recursively copy directory
	function marble_theme_recursive_copy( $src, $dst ) {
		$dir = opendir( $src );
		@mkdir( $dst, 0755, true );
		...
		while ( false !== ( $file = readdir( $dir ) ) ) {
			if ( ( $file != '.' ) && ( $file != '..' ) ) {
				if ( is_dir( $src . '/' . $file ) ) {
					marble_theme_recursive_copy( $src . '/' . $file, $dst . '/' . $file );
				} else {
	...
	// Copy all images from theme to WordPress uploads
	marble_theme_recursive_copy( $theme_images_dir, $wp_uploads_base );
	
	// Mark as copied
	update_option( 'marble_theme_images_copied', true );
	
	// Set a transient to show admin notice
	set_transient( 'marble_theme_images_copied_notice', true, 60 );
}
add_action( 'after_switch_theme', 'marble_theme_copy_images_to_uploads', 5 );

/**
 * Admin notice after images are copied
 */
function marble_theme_images_copied_notice() {
	if ( get_transient( 'marble_theme_images_copied_notice' ) ) {
		?>
		<div class="notice notice-success is-dismissible">
			<p><strong>Marble Theme:</strong> All 182 images have been copied to your WordPress uploads folder and are ready to use!</p>
		</div>
		<?php
		delete_transient( 'marble_theme_images_copied_notice' );
	}
}
add_action( 'admin_notices', 'marble_theme_images_copied_notice' );
